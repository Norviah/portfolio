...
const components: MDXComponents = {
  ...
  a: ({
    ...
    <DynamicLink href={href as string} {...props} />
  ),
  ...
  ol: (
    props: React.DetailedHTMLProps<React.HTMLAttributes<HTMLOListElement>, HTMLOListElement>,
  ) => <OrderedList {...props} />,
  ...
  pre: ({
    ...
    <pre
      className={cn(
        'mb-4 mt-6 overflow-x-auto rounded-lg bg-muted p-5 [&>code]:bg-transparent',
        className,
      )}
      {...props}
    />
  ...
  codeblock: ({ className, ...props }: { className: ClassNameValue }) => (
    <pre
      className={cn('mt mb-4 overflow-x-auto rounded-lg border bg-black py-4', className)}
      {...props}
    />
  ),
